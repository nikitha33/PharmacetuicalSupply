...
function createPharmacies() {
  echo "Enrolling the CA admin"
  mkdir -p organizations/peerOrganizations/pharmacies.auto.com/

  export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/peerOrganizations/pharmacies.auto.com/
  ...
  OrdererOUIdentifier:
    Certificate: cacerts/localhost-11054-ca-pharmacies.pem
    OrganizationalUnitIdentifier: orderer' > "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/msp/config.yaml"

  # Since the CA serves as both the organization CA and TLS CA, copy the org's root cert that was generated by CA startup into the org level ca and tlsca directories

  # Copy pharmacies's CA cert to pharmacies's /msp/tlscacerts directory (for use in the channel MSP definition)
  mkdir -p "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/msp/tlscacerts"
  cp "${PWD}/organizations/fabric-ca/pharmacies/ca-cert.pem" "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/msp/tlscacerts/ca.crt"

  # Copy pharmacies's CA cert to pharmacies's /tlsca directory (for use by clients)
  mkdir -p "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/tlsca"
  cp "${PWD}/organizations/fabric-ca/pharmacies/ca-cert.pem" "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/tlsca/tlsca.pharmacies.auto.com-cert.pem"

  # Copy pharmacies's CA cert to pharmacies's /ca directory (for use by clients)
  mkdir -p "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/ca"
  cp "${PWD}/organizations/fabric-ca/pharmacies/ca-cert.pem" "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/ca/ca.pharmacies.auto.com-cert.pem"
  ...
  echo "Generating the peer0 msp"
  set -x
  fabric-ca-client enroll -u https://peer0:peer0pw@localhost:11054 --caname ca-pharmacies -M "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/pharmacies/ca-cert.pem"
  { set +x; } 2>/dev/null

  cp "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/msp/config.yaml"

  echo "Generating the peer0-tls certificates, use --csr.hosts to specify Subject Alternative Names"
  set -x
  fabric-ca-client enroll -u https://peer0:peer0pw@localhost:11054 --caname ca-pharmacies -M "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/tls" --enrollment.profile tls --csr.hosts peer0.pharmacies.auto.com --csr.hosts localhost --tls.certfiles "${PWD}/organizations/fabric-ca/pharmacies/ca-cert.pem"
  { set +x; } 2>/dev/null

  # Copy the tls CA cert, server cert, server keystore to well known file names in the peer's tls directory that are referenced by peer startup config
  cp "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/tls/tlscacerts/"* "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/tls/ca.crt"
  cp "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/tls/signcerts/"* "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/tls/server.crt"
  cp "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/tls/keystore/"* "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/peers/peer0.pharmacies.auto.com/tls/server.key"

  echo "Generating the user msp"
  set -x
  fabric-ca-client enroll -u https://user1:user1pw@localhost:11054 --caname ca-pharmacies -M "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/users/User1@pharmacies.auto.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/pharmacies/ca-cert.pem"
  { set +x; } 2>/dev/null

  cp "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/users/User1@pharmacies.auto.com/msp/config.yaml"

  echo "Generating the org admin msp"
  set -x
  fabric-ca-client enroll -u https://pharmaciesadmin:pharmaciesadminpw@localhost:11054 --caname ca-pharmacies -M "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/users/Admin@pharmacies.auto.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/pharmacies/ca-cert.pem"
  { set +x; } 2>/dev/null

  cp "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/pharmacies.auto.com/users/Admin@pharmacies.auto.com/msp/config.yaml"
}
